...
/** MarketPulse block 2 — the latest saved analysis shows on load; each click generates (and saves) a new one. */
function AiInsights({ businessId }: { businessId: string }) {
  const analyze = useAnalyzeBusiness()
  const saved = useAnalyses(businessId)
        ...
        <Card className="mt-2 p-4">
          ...
          {history.length > 0 && !analyze.isPending && (
            <details className="mt-4 border-t border-black/5 pt-3 dark:border-white/10">
              <summary className={`cursor-pointer rounded text-sm text-gray-500 dark:text-gray-400 ${FOCUS_RING}`}>
                Análisis anteriores ({history.length})
              </summary>
              <div className="mt-3 space-y-4">
                {history.map((a) => (
                  <details key={a.id} className="rounded-xl bg-black/[0.02] p-3 dark:bg-white/[0.03]">
                    <summary className={`cursor-pointer rounded text-sm text-gray-700 dark:text-gray-300 ${FOCUS_RING}`}>
                      {formatDateTime(a.created_at)}
                      {a.acciones[0] ? ` — ${a.acciones[0].accion}` : ' — sin acciones urgentes'}
                    </summary>
                    <div className="mt-3">
